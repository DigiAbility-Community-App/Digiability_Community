...
// Exported so screens that need a live preview of an in-progress (unsaved)
// preference change — e.g. AccessibilityScreen — can reuse the exact same
// formulas instead of maintaining their own copy that can drift out of sync.
export const getFontScale = (textSize: TextSize) => (size: number) => {
  switch (textSize) {
    case 'Small':
...
export const getThemeColors = (highContrast: boolean): ThemeColors => ({
...
export const getThemeSpacing = (textSize: TextSize): Spacing => {
...
export const AppThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const themeValue = useMemo(() => {
    ...
    const colors = getThemeColors(highContrast);
    const spacing = getThemeSpacing(textSize);

    // Accessible Typography presets
    const typography = {
      heroTitle: {
        fontSize: fs(30),
        lineHeight: fs(38),
